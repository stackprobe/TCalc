...
public:
	...
	void InsertRepeat(int insPos, Element_t e, int count)
	{
		errorCase(insPos < 0 || this->Count < insPos);
		errorCase(count < 0);

		int rPos = this->Count - 1;

		for(int index = 0; index < count; index++)
		{
			this->AddElement(e); // dummy
		}
		int wPos = this->Count - 1;
		...
		for(int index = rPos + 1; index < wPos + 1; index++)
		{
			this->SetElement(index, e);
		}
	}
	void InsertRepeatToStart(Element_t e, int count)
	{
		this->InsertRepeat(0, e, count);
	}
	void RemoveElementsFromStart(int count)
	{
		this->RemoveElements(0, count);
	}
